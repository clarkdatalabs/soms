...
A couple months ago I published a couple examples of making Self Organizing Maps using the R package `kohonen` and player stats from the 2015/2016 NBA season ([available here](https://clarkdatalabs.github.io/soms/SOM_NBA)). That post covered the basics of Self Organizing Maps and the the basic plot types supported by `kohonen`.

Since that post I've been playing with around with text mining and a corpus of the dramas of Shakespeare. My central questions is: 
...
#Downloading Shakespeare's Texts
...
Within `ShakespearePlaysPlus`, there are folders categorizing works by genre. We'll use this as metadata for plots later, so we want to capture it in our table.
```{r, eval=FALSE}
...
We will be using the `tm` library for text mining. The fundemental object we'll be working with is a *corpus*, within which we have one *document* per play.
```{r form corpus}
library("tm")
...
You can see there are lots of stage instructions and other meta-data tags that are all enclosed in angle brackets. To transform the contents of documents within our corpus, we need to define a function that does exactly what we want: strip out anything within angle brackets.
...
Next we'll create what's called a Document Term Matrix.
...
#SOM!
To create our Self Organizing Map, we are first going to create a dataframe with ..
