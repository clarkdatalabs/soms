...
```{r}
character.DTM.modified <- removeSparseTerms(character.DTM, 0.99)
...
```{r}
character.DTM.modified <- character.DTM.modified[wordcount.DTM.modified>29, ]
```
...
```{r}
library(kohonen)
character.SOM <- som(scale(character.DTM.modified), grid = somgrid(25, 25, "hexagonal"), toroidal = FALSE)
```
